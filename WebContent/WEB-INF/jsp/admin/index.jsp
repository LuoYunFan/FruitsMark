...
<div class="layui-layout layui-layout-admin">
            ...
            <ul class="layui-nav layui-nav-tree" lay-filter="test">
                <c:forEach items="${sessionScope.admin.adminRole.roleFuns }" var="f">
                    <c:if test="${ not empty f.funsList }">
                        ...
                        <dl class="layui-nav-child">
                            <c:forEach items="${f.funsList }" var="t">
                                <dd>
                                    <a href="${pageContext.request.contextPath }/${t.funUrl }"
                                       target="${t.funTarget }">${t.funName }</a>
                                </dd>
                            </c:forEach>
                        </dl>
                    </c:if>
                    <c:if test="${empty f.funsList }">
                        <li class="layui-nav-item">
                        <a href="${pageContext.request.contextPath }/${f.funUrl }"
                           target="${f.funTarget }">${f.funName }</a>
                    </c:if>
                    </li>
                </c:forEach>
            </ul>
    ...
    <div class="layui-body">
        <!-- 内容主体区域 -->
        <iframe src="view/welcome" name="myframe" style="width:1200px;height:550px;border: 0;"></iframe>
    </div>
