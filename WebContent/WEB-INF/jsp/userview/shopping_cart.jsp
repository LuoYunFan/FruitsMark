    ...
    <div class="row" style="background-color:white;">
        ...
        <div class="col-sm-10  col-md-10 col-sm-offset-1 col-md-offset-1">
            <form id="cartForm" action="cart/preOrder" method="post">
                ...
                <hr/>
                <div style='text-align:right;width:1000px;height:50px;font-size:16px;color:red;'>已选<span
                        id="totalNum">0</span>件&nbsp;&nbsp;&nbsp;总价：<span id='totalPrice'>0</span>元
                </div>
                <div class="row">
                    <div class="col-lg-4 col-md-4 col-sm-4"></div>
                    <button type="button" class="btn btn-danger btn-lg col-lg-4 col-md-4 col-sm-4"
                            onclick="confirmPre();">确认购买
                    </button>
                </div>
            </form>
        </div>
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function (arr) {
                if (arr != null && arr != "") {
                    ...
                    $("input[name='goodslist']").bind("click", function () {//goodslist复选框点击事件
                        changeTotal(); //修改总价和数量
                        var flag = checkAll(); //如果有一个复选框未被选中，则将全选复选框设置为False
    ...
    function selectAll() {
        var flag = $("#allSelect").prop("checked"); //返回复选框属性的值
        var arr = $("input[name='goodslist']");
        for (var i = 0; i < arr.length; i++) {
            $(arr[i]).prop("checked", flag);//设置复选框的属性checked
        }
        changeTotal();
    }
    ...
    function changeTotal() { //计算所选商品的总价及数量
        var arr = $("input[name='goodslist']");
        var totalPrice = 0;
        var num = 0;
        for (var i = 0; i < arr.length; i++) {
            if ($(arr[i]).prop("checked")) {
                num++;
                var cartId = $(arr[i]).val(); //返回value属性的值
